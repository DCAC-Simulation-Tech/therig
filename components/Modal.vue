<template>
    <div class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center shadow-inner">
      <div class="bg-white p-8 rounded-lg relative w-96">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <button type="submit" class="bg-[rgb(255,104,0)] text-white px-4 py-2 rounded w-full">Submit</button>
        </form>
      </div>
    </div>
  </template>
...
const handleSubmit = async () => {
  try {
    await axios.post('/api/join', form.value)
